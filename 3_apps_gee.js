...
//Change style of cursor to 'crosshair' //creu for inspecting
...
ui.root.insert(1,panel)   // or panel.add(intro) but not is the same
...
// The following function creates and styles 1 row of the legend.
...
// Add color and names to legend using a loop
for (var i = 0; i < 3; i++) {
  extentLegend.add(makeRowa(paletteMAPa[i], namesa[i]));
  }  


//Height Legend => Linear legend
///////////////
...
Map.onClick(function(coords){
  ...
  inspector.clear();
  inspector.style().set('shown', true); //display pannel
  inspector.add(ui.Label('Loading...', {color: 'gray'}));
    
  //Computer the HBA value
  var point = ee.Geometry.Point(coords.lon, coords.lat);
  var reduce = hba.reduce(ee.Reducer.first()); //value of the clicked pixel
  var sampledPoint = reduce.reduceRegion(ee.Reducer.first(), point, 30);//get the value to this point with a buffer of 30
  ...
  inspector.clear();
      ...
      inspector.add(ui.Label({
          value: 'HBA: ' + result.toFixed(2), // 2 decimals
